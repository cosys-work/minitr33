...
export class GraForceService implements AfterViewInit {
  ...
  getGraphDataSets(): Array<Described<(graph: any) => any>> {
    ...
    const loadD3Dependencies = async (graph: ForceGraph3DInstance) => {
      const r = await fetch('./assets/csv/d3Deps.csv');
      const v = await r.text();
      const data = csvParse(v);
      const nodes: { path: any; leaf: any; module: any; size: number; }[] = [];
      const links: { source: any; target: any; }[] = [];
      data.forEach((v_2: any) => {
        const { size, path } = v_2;
        const levels = path.split('/'), module = levels.length > 1 ? levels[1] : null, leaf = levels.pop(), parent = levels.join('/');
